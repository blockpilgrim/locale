...
// =============================================================================
// Pentagon geometry — polarToCartesian vertex calculation
// =============================================================================
// Tests the shared pentagon geometry from src/lib/pentagon.ts which is used by
// both VibeSpectrum (React) and the social card Satori renderer.

import { polarToCartesian } from "@/lib/pentagon";

describe("Pentagon geometry (polarToCartesian)", () => {
  ...
  it("scores [0,0,0,0,0] produce vertices all at center", () => {
    for (let i = 0; i < 5; i++) {
      const pt = polarToCartesian(cx, cy, 0, i); // radius 0 for score 0
      expect(pt.x).toBeCloseTo(cx, 5);
      expect(pt.y).toBeCloseTo(cy, 5);
    }
  });

  it("scores [100,100,100,100,100] produce a regular pentagon", () => {
    const points = Array.from({ length: 5 }, (_, i) =>
      polarToCartesian(cx, cy, radius, i),
    );

    // All sides of a regular pentagon should be equal length
  ...
  it("individual axis scores only affect their own vertex", () => {
    // Compare vertex positions: score 50 on index 2 vs score 100 on index 2
    const halfPt = polarToCartesian(cx, cy, radius * 0.5, 2);
    const fullPt = polarToCartesian(cx, cy, radius, 2);

    // The half-score point should be halfway between center and full
    expect(halfPt.x).toBeCloseTo((cx + fullPt.x) / 2, 5);
    expect(halfPt.y).toBeCloseTo((cy + fullPt.y) / 2, 5);

    // Other vertices are unaffected by the score on index 2
    const otherFull = polarToCartesian(cx, cy, radius, 0);
    const otherHalf = polarToCartesian(cx, cy, radius, 0); // same radius, not affected
    expect(otherFull.x).toBeCloseTo(otherHalf.x, 5);
    expect(otherFull.y).toBeCloseTo(otherHalf.y, 5);
  });

  it("vertices are arranged clockwise", () => {
    const points = Array.from({ length: 5 }, (_, i) =>
      polarToCartesian(cx, cy, radius, i),
    );

    // Compute cross product of consecutive edge vectors
    // For clockwise arrangement in SVG (y-axis points down), the cross product
    // sign should be consistent (positive for CW in screen coords)
    // Note: In SVG, y increases downward, so "clockwise" in screen space
    // means cross products should be negative (standard math convention)
    // or consistently signed.
    ...
    for (let i = 1; i < signs.length; i++) {
      if (signs[i] !== signs[0]) allSameSign = false;
    }
