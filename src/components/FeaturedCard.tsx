...
export function FeaturedCard({ report }: { report: FeaturedReport }) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className="group relative overflow-hidden rounded-2xl border border-border-light bg-surface p-6 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 text-left w-full disabled:opacity-70 disabled:cursor-wait"
    >
      {/* Top accent gradient */}
