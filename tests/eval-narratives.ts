#!/usr/bin/env tsx
// ---------------------------------------------------------------------------
// Golden Dataset Prompt Evaluation Script (T7.1)
// ---------------------------------------------------------------------------
// Generates AI narrative prompts for 20 diverse US addresses and optionally
// calls the Anthropic API to produce actual narratives.
//
// Usage:
//   npx tsx tests/eval-narratives.ts          # Output prompts only (default)
//   npx tsx tests/eval-narratives.ts --live   # Call AI and generate narratives
//
// Outputs:
//   tests/output/prompts/<label>.txt           # Constructed prompts
...
async function main() {
  ...
  // Write the system prompt once (shared across all addresses)
  ...
  for (const addr of goldenAddresses) {
    ...
    if (isLive) {
      try {
        ...
        console.error(`  FAILED: ${addr.label}:`, err);
      }
    }
  }
  ...
  if (isLive) {
    ...
    console.log("Run with --live to generate AI narratives (requires ANTHROPIC_API_KEY).");
  }
}
