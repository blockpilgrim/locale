...
- [ ] **T0.2 — Database setup + schema**
  Install Drizzle ORM + `@vercel/postgres` (or `postgres` driver for local dev). Define schema in `lib/db/schema.ts`: `locations`, `reports` (with JSONB `data` column, `status` enum, unique `slug`), optional `search_queries`. Write migration script. Add a `lib/db/index.ts` connection helper. Verify migration runs against a local Postgres instance.
  - *Depends on:* T0.1
