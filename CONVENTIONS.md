...
- **Synchronous narrative generation:** The route handler awaits `generateNarrative()` before returning the response. This ensures the narrative is persisted to the DB before the client navigates to the report page. The POST takes longer (~10-15s) but the report is immediately ready.
- **Anti-pattern: fire-and-forget promises in route handlers.** Do NOT use detached `promise.catch()` or `after()` for critical background work in Next.js route handlers. Both are unreliable: fire-and-forget promises can be dropped after the response is sent, and `after()` may not execute callbacks in dev mode. If work must complete, `await` it before returning the response.
- **Stale "generating" recovery:** The cache hit path detects reports stuck in "generating" for > 60 seconds and re-runs narrative generation synchronously before responding.
