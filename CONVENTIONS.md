...
- **Atomic JSONB merge:** When updating a subset of a JSONB column (e.g., adding `archetype` to `data`), use the Postgres `||` operator via Drizzle's `sql` tagged template (`data = ${reports.data} || ${json}::jsonb`) to avoid read-modify-write race conditions. Never read then write the whole JSONB object.
- **Shared geometry/math modules:** Pure computation functions shared between React components and Satori card renderers live in `src/lib/` (e.g., `src/lib/pentagon.ts`). This prevents divergence between page and card rendering.
...
- **`generateMetadata`:** Used on the report page for dynamic OG tags. Shares the `fetchReport()` helper with the page component to avoid duplicate DB calls (Next.js deduplicates within a single render). OG image: if archetype data exists, uses the `/api/report/[slug]/card?format=og` route (Satori-generated card via `@vercel/og`); otherwise falls back to a Mapbox Static Images URL with pin overlay. Requires `NEXT_PUBLIC_BASE_URL` (or `VERCEL_URL`) for absolute OG image URLs.
